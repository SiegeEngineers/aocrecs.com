...
async def get_person(context, person_id):
    ...
    person_query = """
        ...
        twitch, mixer, douyu, youtube, discord, birthday, date_part('year', age(birthday)) as age, liquipedia
        from people
        where id=:person_id
    """
    account_query = """
    ...
    return dict(
        person,
        portrait_link=f"/api/portrait/{person['id']}" if person['has_portrait'] else None,
        accounts=[
            dict(
                id=a['id'],
                name=a['name'],
